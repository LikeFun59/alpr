...
# Возвращает количество результатов + результаты в виде полей
def detect_with_yolo(preloaded_model: YOLO, car_image: Image, verbose: bool) -> (int, any): # type: ignore
...
# Эта функция должна быть вызвана сразу после получения полей номерных знаков
...
# Читаем поле с одним номерным знаком
# Возвращаем номерной знак в виде строки
def read_license_plate(unique_identifier: str, box: any, original_image: Image, width_boost: int, additional_white_spacing_each_side: int, debug: bool, should_try_lp_crop: bool, minimum_number_of_chars_for_match: int) -> (Image, str): # type: ignore
    ...
    license_plate_cropped_img = original_image.crop(  # обрезка по номерному знаку
        (x_min, y_min, x_max, y_max)
    ).convert( # черно-белые изображения делают предварительную обработку более эффективной
        "L"
    ).resize( # изменение размера делает распознавание более эффективным
        [boosted_width, boosted_height]
    )
    if should_try_lp_crop:
        # обрезка слева и справа, поскольку распознавание номерного знака совпадает с переполнением
    ...
    # Формируем текст в виде текста
    letter_rectangles = get_letter_rectangles_from_contours(iwl_wb)
    iwl_wb_pil = Image.fromarray(iwl_wb, mode="L")
    
    # Нет смысла пытаться читать, если даже прямоугольников недостаточно (пропускаем чтение, что повышает производительность)
    ...
    def process_letter_rectangle(args):
        ...
        char_from_img = pytesseract.image_to_string(new_letter_box_img, lang="eng", config="--psm 13 --dpi 96 -c tessedit_char_whitelist=ABEKMHOPCTYX0123456789")
